...
const usage = `Usage: node scripts/run-codex-review.mjs <plan|impl> <session-key> [--fresh]
...
const repoRoot =
  existsSync(path.join(sourceRepoRoot, 'sync.sh')) && existsSync(path.join(sourceRepoRoot, 'skills'))
    ? sourceRepoRoot
    : projectRepoRoot;
const stateDir = process.env.CODEX_REVIEW_STATE_DIR
  ? path.resolve(process.env.CODEX_REVIEW_STATE_DIR)
  : path.join(repoRoot, '.codex-review-state');
const schemaCandidates = [
  path.join(scriptDir, 'review-schema.json'),
  path.join(repoRoot, '.agents', 'scripts', 'review-schema.json'),
...
function terminateCodexProcess(child) {
...
function destroyCodexPipes(child) {
  child.stdin.destroy();
  child.stdout.destroy();
  child.stderr.destroy();
}

function isCodexCliPanicLine(line) {
  return /panicked at|Could not create otel exporter|Attempted to create a NULL object/i.test(line);
}
...
async function runCodex(
  ...
  const forceFinishSoon = (exitCode) => {
  ...
  const timeout = setTimeout(() => {
    timedOut = true;
    const message = `[run-codex-review] timed out after ${timeoutSeconds}s; terminating codex process tree`;
    chunks.push(`${message}\n`);
  ...
  const onChunk = (buffer) => {
    ...
    for (const line of text.split(/\r?\n/)) {
      ...
      try {
        ...
        if (!fatalStartupOutput && isCodexCliPanicLine(line)) {
          fatalStartupOutput = true;
          const message =
            '[run-codex-review] fatal Codex CLI startup output observed; terminating codex process tree';
          chunks.push(`${message}\n`);
          process.stderr.write(`${message}\n`);
          terminateCodexProcess(child);
          destroyCodexPipes(child);
...
async function emitFailureSummary(context, logText, sessionKeyForLog) {
  const patterns = [
    ...
    ['codex-cli-panic', /panicked at|Could not create otel exporter|Attempted to create a NULL object/i],
